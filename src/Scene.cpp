...
void Scene::init() {
  ...
  mvpComponent->setProjection(static_cast<float>(window.getWidth()) /
                              static_cast<float>(window.getHeight()));

  setViewCamera();

  cubeMesh =
      std::make_unique<Cube>(std::move(cubeRender), std::move(mvpComponent),
                             std::move(cubeGrassTexture), std::move(cubeShader),
                             std::move(cubeBuffer));
...
void Scene::handleInput() { camera->handleInput(window); }

void Scene::setViewCamera() {
  mvpComponent->setViewFromCamera(
      camera->getCameraPos(), camera->getCameraPos() + camera->getCameraFront(),
      camera->getCameraUp());
}
